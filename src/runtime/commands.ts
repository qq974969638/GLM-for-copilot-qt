...
export function registerCommands(context: vscode.ExtensionContext): void {
	context.subscriptions.push(
		...
		// fork's defaults stay aligned with upstream, and Coding Plan users opt
		// in explicitly.
 ...
 * fork's built-in defaults aligned with upstream while letting Coding Plan
 * users opt into the fork's preferred route + vision mode + MCP setup with
 * one command.
 *
 * Writes:
 *   - modelManagement (merged onto effective config):
